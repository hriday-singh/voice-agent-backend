...
import { createNewAgent } from "../../utils/agentOperations";
...
const AgentCreate: React.FC = () => {
  ...
  const [agent, setAgent] = useState<Partial<AgentDetail>>(defaultAgent);
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Check if agent_type has the correct format (lowercase, numbers, underscores only)
      const agentTypeRegex = /^[a-z0-9_]+$/;
      if (!agentTypeRegex.test(agent.agent_type)) {
        setError(
          "Agent type must contain only lowercase letters, numbers, and underscores"
        );
        setLoading(false);
        return;
      }
      ...
      if (agent.is_outbound && !agent.startup_message) {
        setError("Startup message is required for outbound agents");
        setLoading(false);
        return;
      }

      if (!agent.languages?.primary) {
      ...
      if (
        !agent.languages?.supported ||
        agent.languages.supported.length === 0
      ) {
        setError("At least one supported language is required");
        setLoading(false);
        return;
      }

      // Ensure speech_context and limitations are arrays
      if (!agent.speech_context) {
        setAgent((prev) => ({ ...prev, speech_context: [] }));
      }

      if (!agent.limitations) {
        setAgent((prev) => ({ ...prev, limitations: [] }));
      }
  ...
  // JSX return with all the complete form sections
  return (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder="unique_lowercase_with_underscores"
                  className="w-full px-3 py-2 border border-[#e7e2d3] rounded-md"
                />
